...
bool Node::AddNeighbor(const std::pair<const Node*, float>& neighbor){
...
bool Node::IsNeighbor(const Node* node_ptr) {
	return !(m_WeightedNeighbors.find(node_ptr)
			 == m_WeightedNeighbors.end());
			
}

bool Node::SetFlag(const std::string& flag_label, int flag) {
	return m_Flags.insert(std::pair<const std::string, int>(flag_label, flag)).second;
}
